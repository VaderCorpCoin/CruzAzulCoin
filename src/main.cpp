...
// Copyright (c) 2011-2013 The CruzAzul developers
...
const string strMessageMagic = "CruzAzul Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // CruzAzul: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // CruzAzul: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // CruzAzul: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// CruzAzul: find block wanted by given orphan block
...
int64 GetProofOfWorkReward(unsigned int nBits)
{
    ...
    // CruzAzul: subsidy is cut in half every 16x multiply of difficulty
...
// CruzAzul: miner's coin stake is rewarded based on coin age spent (coin-days)
...
// CruzAzul: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // CruzAzul: target change every block
    // CruzAzul: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // CruzAzul: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // CruzAzul: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // CruzAzul: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // CruzAzul: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // CruzAzul: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // CruzAzul: track money supply and mint amount info
    ...
    // CruzAzul: fees are not collected by miners as in bitcoin
    // CruzAzul: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// CruzAzul: total coin age spent in transaction, in the unit of coin-days.
...
// CruzAzul: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // CruzAzul: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // CruzAzul: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // CruzAzul: record proof-of-stake hash value
    ...
    // CruzAzul: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // CruzAzul: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // CruzAzul: coinbase output should be empty if proof-of-stake block
    ...
    // CruzAzul: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // CruzAzul: check that the block satisfies synchronized checkpoint
    ...
    // CruzAzul: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // CruzAzul: verify hash target and signature of coinstake tx
    ...
    // CruzAzul: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // CruzAzul: getblocks may not obtain the ancestor block rejected
    ...
    // CruzAzul: if responsible for sync-checkpoint send it
...
// CruzAzul: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "CruzAzul", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "CruzAzul", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // CruzAzul: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // CruzAzul: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // CruzAzul: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // CruzAzul: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // CruzAzul: should not enter safe mode for longer invalid chain
    // CruzAzul: if sync-checkpoint is too old do not enter safe mode
    ...
    // CruzAzul: if detected invalid checkpoint enter safe mode
    ...
    // CruzAzul: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // CruzAzul: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // CruzAzul: record my external IP reported by peer
        ...
        // CruzAzul: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // CruzAzul: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // CruzAzul: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // CruzAzul: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // CruzAzul: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // CruzAzul: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
