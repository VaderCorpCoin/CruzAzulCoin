...
// CruzAzul: optional setting to unlock wallet for block minting only;
...
// CruzAzul: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // CruzAzul: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // CruzAzul: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // CruzAzul: not avatar mode
                        {
...
// CruzAzul: create coin stake transaction
...
// CruzAzul: check 'spent' consistency between wallet and txindex
// CruzAzul: fix wallet spent state according to txindex
...
// CruzAzul: disable transaction (only for coinstake)
